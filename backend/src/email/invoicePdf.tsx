...
function InvoicePdfDocument({ invoice }: InvoicePDFProps) {
  const hasTax = invoice.taxRate ? parseFloat(dec(invoice.taxRate)) > 0 : false;
  const hasDiscount = invoice.discount ? parseFloat(dec(invoice.discount)) > 0 : false;
          ...
          {invoice.lineItems.map((item, i) => (
            ...
            >
              ...
              <Text style={[styles.tableCellMono, styles.colRate]}>
                {parseFloat(dec(item.rate)).toFixed(2)}
              </Text>
              <Text style={[styles.tableCellMono, styles.colAmt]}>
                {parseFloat(dec(item.amount)).toFixed(2)}
              </Text>
            </View>
          ))}
        </View>
          ...
          <View style={styles.totalsBox}>
            <View style={styles.totalsRow}>
              ...
              <Text style={styles.totalsValue}>
                {parseFloat(dec(invoice.subtotal)).toFixed(2)} {invoice.currency}
              </Text>
            </View>
            {hasTax ? (
              <View style={styles.totalsRow}>
                <Text style={styles.totalsLabel}>Tax ({dec(invoice.taxRate)}%)</Text>
                <Text style={styles.totalsValue}>
                  {parseFloat(dec(invoice.taxAmount)).toFixed(2)} {invoice.currency}
                </Text>
              </View>
            ) : null}
            {hasDiscount ? (
              <View style={styles.totalsRow}>
                ...
                <Text style={styles.totalsValue}>
                  -{parseFloat(dec(invoice.discount)).toFixed(2)} {invoice.currency}
                </Text>
              </View>
            ) : null}
            <View style={styles.totalsFinalRow}>
              <Text style={styles.totalsFinalLabel}>Total Due</Text>
              <Text style={styles.totalsFinalValue}>{fmt(dec(invoice.total), invoice.currency)}</Text>
            </View>
          </View>
        </View>
